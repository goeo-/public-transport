    ...
    where S: ser::Serializer
{
    ...
    fn visit_str<E>(self, v: &str) -> Result<S::Ok, E>
        where E: de::Error
    {
        match cid::Cid::from_str(v) {
            Ok(cid) => {
                let cid_bytes = cid.to_bytes();
                match cid_bytes[..2] {
                    [0x01, 0x55] => {
                        let mut cid_bytes = cid_bytes.clone();
                        cid_bytes.push(0);
                        unsafe {
                            let cid_cstr = CStr::from_bytes_with_nul_unchecked(&cid_bytes);
                            self.0.serialize_newtype_struct(cid::serde::CID_SERDE_PRIVATE_IDENTIFIER, cid_cstr).map_err(s2d)
                        }
                    },
                    _ => {return self.0.serialize_str(&v).map_err(s2d);}
                }
            },
            Err(_) => self.0.serialize_str(v).map_err(s2d)
        }
    }
